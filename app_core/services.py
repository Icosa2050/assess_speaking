...
from app_core.app_data import build_app_data_paths, resolve_reports_dir
...
DEFAULT_WHISPER_OPTIONS = ("tiny", "base", "small", "medium", "large-v3-turbo", "large-v3")
NEW_LANGUAGE_OPTION = "__new_language__"
BOOTSTRAP_KEY = "_vostavo_core_bootstrapped"
SKIP_BOOTSTRAP_ENV_VAR = "VOSTAVO_SKIP_BOOTSTRAP"
...
def build_provider_connection(
    *,
    ...
    if connection.provider_kind == "openrouter":
        metadata = {
            "http_referer": normalize_openrouter_http_referer(
                openrouter_http_referer,
                require_valid=True,
            ),
            "app_title": normalize_openrouter_app_title(openrouter_app_title),
        }
...
def build_support_bundle_request(
    ...
    *,
    ...
    from app_core.diagnostics import collect_support_bundle_diagnostics
...
def hydrate_state_from_storage(state) -> Any:
    if os.environ.get(SKIP_BOOTSTRAP_ENV_VAR) == "1":
        return state
    ...
    state.prefs.llm_api_key = ""
    state.prefs.openrouter_http_referer = normalize_openrouter_http_referer(
        prefs.get("openrouter_http_referer") or state.prefs.openrouter_http_referer
    )
    state.prefs.openrouter_app_title = normalize_openrouter_app_title(
        prefs.get("openrouter_app_title") or state.prefs.openrouter_app_title
    )
...
def review_summary(payload: dict | None) -> dict[str, Any]:
    ...
    return {
        ...
        "gates": {
            ...
            "content_validity_pass": _gate_value(checks, "content_validity_pass"),
