...
from app_core.diagnostics import StartupDiagnostic
from app_core.state import AppPreferences, AppState, ProviderConnection
from app_core.state import DraftSession
...
class AppShellServiceTests(unittest.TestCase):
    def test_build_provider_connection_rejects_invalid_openrouter_referer(self):
        with self.assertRaisesRegex(ValueError, "OpenRouter HTTP Referer"):
            build_provider_connection(
                ...
                api_key="key-123",
                openrouter_http_referer="test-referer",
                openrouter_app_title="Vostavo",
            )

    @mock.patch.dict(
        os.environ,
        {"VOSTAVO_SKIP_BOOTSTRAP": "", "LLM_API_KEY": "", "OPENROUTER_API_KEY": "", "OLLAMA_API_KEY": ""},
        clear=False,
    )
    @mock.patch("app_core.services.load_history_records")
    @mock.patch("app_core.services.load_theme_library")
    @mock.patch("app_core.services.load_workspace_prefs")
    ...
    def test_hydrate_state_from_storage_preserves_explicit_openrouter_app_title(
        ...
        hydrated = hydrate_state_from_storage(AppState(prefs=AppPreferences()))
    ...
    @mock.patch.dict(
        os.environ,
        {"VOSTAVO_SKIP_BOOTSTRAP": "", "OPENROUTER_API_KEY": "env-key", "LLM_API_KEY": ""},
        clear=False,
    )
    @mock.patch("app_core.services.load_history_records")
    @mock.patch("app_core.services.load_theme_library")
    ...
    @mock.patch.dict(os.environ, {"VOSTAVO_SKIP_BOOTSTRAP": ""}, clear=False)
    @mock.patch("app_core.services.load_report_payload")
    @mock.patch("app_core.services.load_history_records")
    @mock.patch("app_core.services.load_theme_library")
    @mock.patch("app_core.services.load_workspace_prefs")
  ...
  "speaker_profiles": {
    "bern": {
      ...
      "target_duration_sec": 90
            ...
            from app_core.services import save_state_preferences
    ...
    @mock.patch("app_core.services.set_secret")
    ...
    @mock.patch("app_core.services.secret_store_status")
    @mock.patch("app_core.services.resolve_connection_runtime")
    ...
    @mock.patch("app_core.diagnostics.collect_startup_diagnostics")
    @mock.patch("app_core.services.secret_store_status")
    @mock.patch("app_core.services.resolve_connection_runtime")
    ...
    @mock.patch("app_core.services.backend_client.create_support_bundle")
    @mock.patch("app_core.services.build_support_bundle_request")
    def test_create_support_bundle_archive_uses_backend_client_with_log_dir(
        ...
        state = AppState(prefs=AppPreferences(log_dir="/tmp/vostavo-support"))
    ...
    @mock.patch("app_core.services.backend_client.download_support_bundle")
    @mock.patch("app_core.services.create_support_bundle_archive")
    ...
    def test_export_support_bundle_archive_rejects_missing_bundle_id(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch(
            "app_core.services.create_support_bundle_archive",
            return_value=mock.Mock(bundle_id=""),
        ), mock.patch("app_core.services.backend_client.download_support_bundle") as download:
            with self.assertRaisesRegex(RuntimeError, "Support bundle could not be created"):
    ...
    @mock.patch("app_core.services.whisper_model_status", return_value={"cached": True, "cached_path": "/tmp/model"})
    @mock.patch("app_core.services.ensure_model_downloaded")
    ...
    @mock.patch("app_core.services.time.sleep")
    @mock.patch("app_core.services.backend_client.get_assessment_status")
    ...
    @mock.patch("app_core.services.backend_client.upload_audio_path", side_effect=RuntimeError('{"code":"backend_unavailable","detail":"offline"}'))
    ...
    @mock.patch(
        "app_core.services.backend_client.load_history_detail",
    ...
    @mock.patch("app_core.services.backend_client.load_samples")
    def test_list_sample_trials_filters_language_and_level(self, mock_load_samples):
        ...
        with mock.patch("app_core.services.Path.exists", return_value=False):
    ...
    @mock.patch("app_core.services.backend_client.load_samples", side_effect=RuntimeError("offline"))
    def test_list_sample_trials_falls_back_to_local_samples(self, _mock_load_samples):
        with mock.patch("app_core.services._local_sample_trials", return_value=[{"sample_id": "en_B1_travel_story", "language": "en", "cefr": "B1", "title": "travel story", "path": "/tmp/en.wav"}]), \
                mock.patch("app_core.services.Path.exists", return_value=True):
