...
from app_core import backend_client
...
class BackendClientTests(unittest.TestCase):
    def test_backend_base_url_reuses_state_or_starts_backend(self):
        with mock.patch("app_core.backend_client.get_backend_state", return_value={"base_url": "http://ready"}) as get_state, \
                mock.patch("app_core.backend_client.ensure_local_backend") as ensure_backend:
        ...
        with mock.patch("app_core.backend_client.get_backend_state", return_value=None), \
                mock.patch("app_core.backend_client.ensure_local_backend", return_value={"base_url": "http://started"}) as ensure_backend:
    ...
    def test_request_wraps_structured_backend_status_errors(self):
        ...
        with mock.patch("app_core.backend_client.backend_base_url", return_value="http://backend.local"), \
                mock.patch("app_core.backend_client.httpx.request", return_value=response):
    ...
    def test_request_wraps_unreachable_backend_errors(self):
        ...
        with mock.patch("app_core.backend_client.backend_base_url", return_value="http://backend.local"), \
                mock.patch(
                    "app_core.backend_client.httpx.request",
    ...
    def test_endpoint_helpers_parse_responses_and_forward_payloads(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("app_core.backend_client._request", side_effect=responses) as request:
    ...
    def test_backend_path_segments_are_percent_encoded(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch(
            "app_core.backend_client.backend_base_url",
            return_value="http://backend.local",
        ), mock.patch("app_core.backend_client.httpx.request", side_effect=fake_request):
    ...
    def test_download_support_bundle_parses_encoded_content_disposition_filename(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("app_core.backend_client._request", return_value=response):
    ...
    def test_load_history_detail_rejects_invalid_payload_shape(self):
        with mock.patch("app_core.backend_client._request", return_value=_FakeResponse({"payload": []})):
            with self.assertRaises(RuntimeError) as raised:
    ...
    def test_collection_helpers_tolerate_non_mapping_payloads(self):
        with mock.patch("app_core.backend_client._request", side_effect=[_FakeResponse([]), _FakeResponse([])]):
