...
class StartupDiagnosticsTests(unittest.TestCase):
    ...
    @mock.patch("app_shell.diagnostics.shutil.which", return_value="/opt/homebrew/bin/ffmpeg")
    @mock.patch(
        "app_shell.diagnostics.resolve_connection_runtime",
    ...
    @mock.patch(
        "app_shell.diagnostics.build_client_snapshot",
        return_value={
    ...
    def test_collect_startup_diagnostics_warns_for_repo_local_app_data_override(self, _mock_which, mock_model_availability):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("app_shell.diagnostics.bootstrap_app_environment", return_value=fake_paths):
                state = AppShellState(prefs=AppPreferences(log_dir=str(fake_paths.reports_dir)))
                diagnostics = collect_startup_diagnostics(state, include_runtime_health=False)
    ...
    def test_collect_startup_diagnostics_adds_maintenance_warnings(self, _mock_which, mock_model_availability):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("app_shell.diagnostics.bootstrap_app_environment", return_value=paths):
                diagnostics = collect_startup_diagnostics(
                    AppShellState(prefs=AppPreferences(log_dir=str(paths.reports_dir))),
        ...
        self.assertEqual(by_key["maintenance_tmp"].detail_args["target_page"], "pages/06_Settings.py")
        self.assertEqual(by_key["maintenance_jobs"].detail_args["target_page"], "pages/06_Settings.py")
        self.assertEqual(by_key["maintenance_logs"].detail_args["target_page"], "pages/06_Settings.py")
