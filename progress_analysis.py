...
def task_family_progress(records: Iterable[object], *, speaker_id: Optional[str] = None) -> list[dict]:
    ...
    for family, family_records in sorted(group_by_task_family(records, speaker_id=speaker_id).items()):
        final_scores = [r.final_score for r in family_records if getattr(r, "final_score", None) is not None]
        ...
        rows.append(
            {
                ...
                "latest_final": getattr(latest, "final_score", None),
                "latest_overall": getattr(latest, "overall", None),
